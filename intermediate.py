...
name=sys.argv[1]
if(os.system('python literaltable.py %s'%(name))==0 and os.system('python symboltable.py %s'%(name))==0):
	f=open(name,"r")
	line=f.readlines()
	#print(line)
	...
	for j in range(count,len(line3)):
		...
		if(line3[j].find('dword')>0):
			...
			str9=str4[1][:-1]
			print(str9)			
			if(str9 in reg32):# or reg16 or reg8):
				k=reg32.index(str9)
				line3[j]=line3[j].replace(str(str9),regval1[k])

			if(str9 in reg16):# or reg16 or reg8):
				k=reg16.index(str9)
				line3[j]=line3[j].replace(str(str9),regval2[k])

			if(str9 in reg8):# or reg16 or reg8):
				k=reg8.index(str9)
		...
		if(len(str3)==2):
			if(line3[j].find(',')>0):
				...
				if(str3[1] in symbol):
					...
					line3[j]=line3[j].replace(str3[1],'LIT#'+str(k+1))
			else:
				if(str3[1] in symbol):
					k=symbol.index(str3[1])
					line3[j]=line3[j].replace(str3[1],'sym#'+str(k))
		...
		if(len(str3)==3):
			...
			if(line3[j].find(',')>0):
				...
				if(str3[0] in symbol):
					...
					line3[j]=line3[j].replace(str3[0],'LIT#'+str(k+1))
				if(str3[2] in symbol):
					k=symbol.index(str3[2])
					line3[j]=line3[j].replace(str3[2],'sym#'+str(k+1))
				
				elif(str3[2] in literal):
					k=literal.index(str3[2])
					line3[j]=line3[j].replace(str3[2],'LIT#'+str(k+1))
